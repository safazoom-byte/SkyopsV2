...
export const calculateCredits = (
  ...
  const progEnd = new Date(`${startDate}T12:00:00Z`);
  progEnd.setUTCDate(progStart.getUTCDate() + duration - 1);

  let activeStart = progStart;
  let activeEnd = progEnd;

  if (staff.workFromDate) {
    const wfd = new Date(`${staff.workFromDate}T12:00:00Z`);
    if (wfd > activeStart) activeStart = wfd;
  }
  if (staff.workToDate) {
    const wtd = new Date(`${staff.workToDate}T12:00:00Z`);
    if (wtd < activeEnd) activeEnd = wtd;
  }
  ...
  staffLeaves.forEach((leave) => {
    if (
      ...
      const leaveStart = new Date(`${leave.startDate}T12:00:00Z`);
      const leaveEnd = new Date(`${leave.endDate}T12:00:00Z`);
      const overlapStart = activeStart > leaveStart ? activeStart : leaveStart;
      const overlapEnd = activeEnd < leaveEnd ? activeEnd : leaveEnd;
  ...
  const baseActiveDays = Math.max(0, Math.floor((activeEnd.getTime() - activeStart.getTime()) / (1000 * 60 * 60 * 24)) + 1);

  if (staff.type === "Local") {
    // 5/2 Rule Logic adjusted for leaves
    const netActiveDays = Math.max(0, baseActiveDays - leaveDeduction);
    let grossCredits = Math.round(netActiveDays * (5 / 7));
    // Fallback for short periods (e.g. 1-4 days) to allow utilization
    if (baseActiveDays < 7 && baseActiveDays > 0) {
      grossCredits = Math.ceil(netActiveDays * 0.8);
    }
    return grossCredits;
  } else {
    let grossCredits = 0;
    if (staff.rosterPeriods && staff.rosterPeriods.length > 0) {
      ...
      staff.rosterPeriods.forEach((period) => {
        const pStart = new Date(`${period.start}T12:00:00Z`);
        const pEnd = new Date(`${period.end}T12:00:00Z`);
        const overlapStart = activeStart > pStart ? activeStart : pStart;
        const overlapEnd = activeEnd < pEnd ? activeEnd : pEnd;
        if (overlapStart <= overlapEnd) {
...
export const generateAIProgram = async (
  ...
  for (let i = 0; i < config.numDays; i++) {
    const d = new Date(`${config.startDate}T12:00:00Z`);
  ...
  const programStart = new Date(`${config.startDate}T12:00:00Z`);
  const programEnd = new Date(`${config.startDate}T12:00:00Z`);
  programEnd.setUTCDate(programStart.getUTCDate() + config.numDays - 1);
    ...
    parsed.flights = parsed.flights.map((f: any) => {
      ...
      if (dateField && !/^\d{4}-\d{2}-\d{2}$/.test(dateField)) {
        ...
        if (match) {
          ...
          const pd = new Date(`${dateField}T12:00:00Z`);
          if (!isNaN(pd.getTime())) {
