...
export const CapacityForecast: React.FC<Props> = ({
  ...
  const stats = useMemo(() => {
    ...
    const activeStaff = staff.filter((s) => 
      isStaffActiveForDateRange(s, startDate, endDateStr) && 
      !(s.isDriver || s.isLabour || s.isSecurity || s.isAccountant)
    );
    ...
    localStaff.forEach((s) => {
      // Calculate contracted days within period
      let contractDays = duration;
      if (s.workFromDate || s.workToDate) {
        const cStart = s.workFromDate ? new Date(`${s.workFromDate}T12:00:00Z`) : start;
        const cEnd = s.workToDate ? new Date(`${s.workToDate}T12:00:00Z`) : end;
        contractDays = getOverlapDays(start, end, cStart, cEnd);
      }
      ...
      sLeaves.forEach((l) => {
        const lStart = new Date(`${l.startDate}T12:00:00Z`);
        const lEnd = new Date(`${l.endDate}T12:00:00Z`);
        const cStart = s.workFromDate ? new Date(`${s.workFromDate}T12:00:00Z`) : start;
        const cEnd = s.workToDate ? new Date(`${s.workToDate}T12:00:00Z`) : end;
        
        const activeStart = start > cStart ? start : cStart;
        const activeEnd = end < cEnd ? end : cEnd;
    ...
    rosterStaff.forEach((s) => {
      ...
      if (s.workFromDate) {
        const wfd = new Date(`${s.workFromDate}T12:00:00Z`);
        if (wfd > activeStart) activeStart = wfd;
      }
      if (s.workToDate) {
        const wtd = new Date(`${s.workToDate}T12:00:00Z`);
        if (wtd < activeEnd) activeEnd = wtd;
      }

      if (s.rosterPeriods && s.rosterPeriods.length > 0) {
        s.rosterPeriods.forEach((p) => {
          const pStart = new Date(`${p.start}T12:00:00Z`);
          const pEnd = new Date(`${p.end}T12:00:00Z`);
          days += getOverlapDays(activeStart, activeEnd, pStart, pEnd);
        });
      } else {
        days = getOverlapDays(start, end, activeStart, activeEnd);
      }
