...
import { Staff, ShiftConfig, LeaveRequest, Skill, isStaffActiveForDateRange } from "../types";
...
export const StationStatistics: React.FC<Props> = ({
  ...
  const stats = useMemo(() => {
    ...
    const filteredStaff = staff.filter(s => isStaffActiveForDateRange(s, startDate, endDate) && !(s.isDriver || s.isLabour || s.isSecurity || s.isAccountant));
